...
if __name__ == "__main__":
    ...
    for ntr in range(1,ntrajs+1):
        pop_file = "traj.{}/state.dat".format(ntr)
        cwd = os.getcwd()
        ...
        with open(popfile,'r') as pf:
            for line in pf:
                if "CurrentState" in line or "#" in line:
                    ...
                    state = int(line.split()[1])   # -1 for abin code or if states are number from 1 (GS). REASON: arrays with state as dim starts from 0
                    #print(state)
